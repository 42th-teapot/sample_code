 ...
 * 
 * A sample code from data structure course
 * Please note that I write the get(),put(),restructure methods, and remove method
 * The helper functions are written by my professor(John Donaldson) of this course.
 */
...
public class RBTreeMap<K extends Comparable<? super K>,V> implements MapADT<K,V> {
	...
	if (this.isEmpty())
	    ...
	    int c = searchKey.compareTo(this.key);
	    if (c == 0)
			return this.value;
	    else if (c > 0)
    ...
    protected V put(K key, V value, RBTreeMap<K,V> parent, RBTreeMap<K,V> gran){
	   ...
	   if (parent != null)
	   this.color = RED;
	   this.left = new RBTreeMap<K, V>();
	   this.right = new RBTreeMap<K, V>();
       }
       else {
          ...
          if(comp==0){  // base case II:  key is already present
              ...
              rvalue = this.left.put(key, value, this, parent);
	  }
	  else {  // recursive case II: put in right subtree
	      rvalue = this.right.put(key, value, this, parent);
	  }
       }
	   ...
	   // Encounter consecutive red nodes
	   ...
	   if(uncle.color==RED){
	       ...
	       // adjustment case III:  the uncle is black, so rotate
	       int comp1 = this.key.compareTo(parent.key);
	       int comp2 = parent.key.compareTo(gran.key);
	       if (comp1 < 0 && comp2 < 0) {
		   rotateRight(gran);
    ...
    //----------------------------------------------------------------------
    // FROM HERE TO THE END: written by my professor of this course
    //----------------------------------------------------------------------
